...
def show_search_page():
    ...
    if len(restricted_df) > 0:
        ...
        # Add manual permit selector below the grid
        st.markdown("---")
        st.subheader("Select Permit for Details")
        
        col1, col2 = st.columns([3, 1])
        with col1:
            # Create permit options with facility names
            permit_options = ['Select a permit...'] + [
                f"{row['Permit Number']} - {row['Facility']}" 
                for _, row in permit_summary.iterrows()
            ]
            
            selected_option = st.selectbox(
                "Choose a permit to view details:",
                options=permit_options,
                key="manual_permit_select"
            )
        
        with col2:
            if selected_option != 'Select a permit...':
                # Extract permit number
                selected_permit_num = selected_option.split(' - ')[0]
                
                # Debug the button click
                if st.button("View Details", type="primary", use_container_width=True):
                    st.write(f"DEBUG: Button clicked for permit {selected_permit_num}")
                    
                    # Find facility name
                    selected_row = permit_summary[permit_summary['Permit Number'] == selected_permit_num]
                    st.write(f"DEBUG: Found row: {not selected_row.empty}")
                    
                    if not selected_row.empty:
                        facility_name = selected_row.iloc[0]['Facility']
                        st.write(f"DEBUG: Setting session state - Permit: {selected_permit_num}, Facility: {facility_name}")
                        
                        st.session_state.selected_permit = selected_permit_num
                        ...
                        st.error("Could not find permit data")
...
def show_details_page():
    ...
    <div style='background-color: #1a365d; color: white; margin: -1rem calc(50% - 50vw) 0; padding: 24px 32px; border-bottom: 3px solid #2d3748;'>
        <div style='max-width: 1400px; margin: 0 auto;'>
            ...
            <div style='display: grid; grid-template-columns: repeat(4, 1fr); gap: 24px; margin-top: 16px;'>
                <div style='text-align: center; padding: 12px; background-color: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2);'>
                    <span style='font-size: 24px; font-weight: 700; display: block;'>{len(permit_df)}</span>
                    <span style='font-size: 12px; text-transform: uppercase; letter-spacing: 1px; opacity: 0.8;'>Total Exceedances</span>
                </div>
                <div style='text-align: center; padding: 12px; background-color: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2);'>
                    <span style='font-size: 24px; font-weight: 700; display: block;'>1</span>
                    <span style='font-size: 12px; text-transform: uppercase; letter-spacing: 1px; opacity: 0.8;'>Active Permits</span>
                </div>
                <div style='text-align: center; padding: 12px; background-color: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2);'>
                    <span style='font-size: 24px; font-weight: 700; display: block;'>{len(permit_df[permit_df['Severity'].isin(['Critical', 'High'])])}</span>
                    <span style='font-size: 12px; text-transform: uppercase; letter-spacing: 1px; opacity: 0.8;'>Enforcement Actions</span>
                </div>
                <div style='text-align: center; padding: 12px; background-color: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2);'>
                    <span style='font-size: 24px; font-weight: 700; display: block;'>Non-Compliant</span>
                    <span style='font-size: 12px; text-transform: uppercase; letter-spacing: 1px; opacity: 0.8;'>Current Status</span>
                </div>
            </div>
    ...
    with tab2:
        ...
        # Full exceedance history with enhanced filtering
        ...
        with col1:
            ...
            st.download_button(
                ...
                csv_buffer.getvalue(),
                file_name=f"permit_{permit_num}_exceedances_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
        with col2:
            st.button("Download PDF Report", help="PDF generation would be implemented here")
