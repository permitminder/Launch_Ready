...
<style>
    ...
    .element-container:has(.stButton):nth-child(even) {
    ...
    /* Make facility and permit buttons more interactive */
    button[key*="facility_"], button[key*="permit_"] {
        transition: all 0.2s ease !important;
    }
    
    button[key*="facility_"]:hover, button[key*="permit_"]:hover {
        transform: translateX(2px) !important;
        box-shadow: 2px 2px 8px rgba(0,0,0,0.1) !important;
    }
    
    /* Improved alternating colors for data rows */
    .row-container {
        padding: 0.75rem;
        margin: 0.25rem 0;
        border-radius: 4px;
    ...
    hr {
        ...
        background: linear-gradient(to right, transparent, #e1e4e8, transparent) !important;
    }
    
    /* Highlight row when any button in it is hovered */
    .stButton:hover + .stButton,
    .stButton:has(+ .stButton:hover) {
        opacity: 0.9;
    }
...
def main():
    ...
    if len(restricted_df) > 0:
        ...
        # Add expandable rows for permit details with improved styling
        for idx, row in display_df.iterrows():
            # Add container div for better row styling
            st.markdown(f'<div class="row-container row-{idx % 2}">', unsafe_allow_html=True)
