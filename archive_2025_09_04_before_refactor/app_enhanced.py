...
def main():
    ...
    if not st.session_state.is_paid_user and (is_restricted or full_filtered_count > len(restricted_df)):
        ...
        - Prioritizing records from the last 30 days
    ...
    if len(restricted_df) > 0:
        ...
        # Display summary box
        st.markdown("""
        <div style='background-color: #f8f9fa; padding: 1.5rem; border-radius: 8px; margin-bottom: 1.5rem; border-left: 4px solid #007bff;'>
            <h4 style='margin-top: 0; color: #212529;'>📊 Search Summary:</h4>
        """, unsafe_allow_html=True)
